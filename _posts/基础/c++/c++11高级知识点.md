...
自旋锁（**Spinlock**）是一种**轻量级的线程同步原语**，在高并发/多核环境中有其独特优势，尤其适用于**临界区非常短、小**的场景。
...
3. **嵌入式系统 / 实时系统**
   - 不希望线程阻塞、上下文切换昂贵
4. **用户态同步机制**
   - 用户空间的 lock-free / spin-based 原语实现（比如 TBB、folly）
...
在c++ 函数模板中，一个模板函数的参数（可以是左值，也可以是右值），可以完美的转发给内部的调用函数。完美是指能转发值（value),也能转发其左值还是右值的属性。通过&& 的万能引用和引用折叠来实现转发左值还是右值。折叠的规则：&&& ->&; && && ->&&; &&->&
...
**必须用 `std::forward<T>(arg)`** 来保留它原来的值类别。
...
在模板中**无损地保留实参类型**（左值/右值），通过 `std::forward` 转发到目标函数（语义层面）
...
int&& n = 1000;
wrapper(n); //左值
//因为变量 n 是一个具名变量，即使它是 int&& 类型，它也是一个左值**。
// n 是 int&& 类型 ✅（右值引用）
//但 n 是一个 具名变量 ❌（是左值表达式）
```
